<template>
  <div>
    <Head>
      ...
      <Meta
        name="description"
        content="Search for breweries by name, type, or location."
      />
    </Head>
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-600 dark:text-gray-400">
        Find breweries by name, type, or location.
      </p>
    </div>
    ...
    <template v-else-if="searchResults.length > 0">
      <div class="flex justify-between items-center mb-4">
        ...
        <button
          @click="clearSearch"
          class="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 text-sm"
        >
          Clear Results
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <BreweryCard
          v-for="brewery in searchResults"
          :key="brewery.id"
          :brewery="brewery"
        />
      </div>
    ...
    >
      ...
      <p class="text-gray-600 dark:text-gray-400 mb-6">
        Enter a search term to find breweries.
      </p>
      <div class="flex flex-col sm:flex-row justify-center gap-4">
        <button
          @click="searchPopularTerm('IPA')"
          class="px-4 py-2 bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200 rounded-md transition-colors hover:bg-amber-200 dark:hover:bg-amber-800"
        >
          Search "IPA"
        </button>
        <button
          @click="searchPopularTerm('Craft')"
          class="px-4 py-2 bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200 rounded-md transition-colors hover:bg-amber-200 dark:hover:bg-amber-800"
        >
          Search "Craft"
        </button>
        <button
          @click="searchPopularTerm('Brewing')"
          class="px-4 py-2 bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200 rounded-md transition-colors hover:bg-amber-200 dark:hover:bg-amber-800"
        >
          Search "Brewing"
        </button>
