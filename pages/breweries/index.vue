<template>
  <div>
    <Head>
      <Title>Breweries | Craft Beer</Title>
      <Meta
        name="description"
        content="Browse and filter breweries from around the world."
      />
    </Head>
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-600 dark:text-gray-400">
        Browse and filter breweries from around the world.
      </p>
    </div>
    ...
    <div
      v-else-if="breweries.length === 0"
      class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center"
    >
      <p class="text-gray-600 dark:text-gray-400 mb-4">
        No breweries found with the current filters.
      </p>
      <button
        @click="resetFilters"
        class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md transition-colors"
...
const initFiltersFromQuery = () => {
  ...
  if (filtersRef.value) {
    ...
    if (query.by_type)
      filtersRef.value.filters.by_type = query.by_type as BreweryType;
    if (query.by_city)
      filtersRef.value.filters.by_city = query.by_city as string;
    if (query.by_state)
      filtersRef.value.filters.by_state = query.by_state as string;
    if (query.by_country)
      filtersRef.value.filters.by_country = query.by_country as string;
    if (query.sort)
      filtersRef.value.filters.sort = query.sort as "asc" | "desc";
...
const fetchBreweries = async () => {
  ...
  try {
    ...
    error.value =
      err instanceof Error ? err : new Error("Failed to load breweries");
    breweries.value = [];
  } finally {
    loading.value = false;
  }
