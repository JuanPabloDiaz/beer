...
describe('useBreweryApi', () => {
  ...
  describe('getBreweries', () => {
    ...
    it('should handle errors when fetching breweries', async () => {
      ...
      await expect(getBreweries({})).rejects.toThrow('Failed to fetch breweries: 500 Internal Server Error')
    })
  })
  ...
  describe('getRandomBrewery', () => {
    it('should fetch a random brewery', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ id: 'random123', name: 'Random Brewery' })
      })
    ...
    it('should fetch metadata about breweries', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: () => Promise.resolve({
          total: 8000,
          city_count: 1500,
          type_count: { micro: 4000, brewpub: 2000, regional: 1000, other: 1000 }
        })
      })
